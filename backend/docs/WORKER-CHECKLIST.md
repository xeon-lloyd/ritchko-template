...
- 단일 실행 작업이면 `process.env.NODE_APP_INSTANCE` 같은 프로세스 가드를 둔다.
- 여러 서버에서 동시에 실행될 수 있으면 DB lock, Redis lock, unique key 같은 멱등성 장치를 설계한다.
...
- queue worker면 `util.redis.connect()`가 worker 실행 전에 완료되는지 확인한다.
