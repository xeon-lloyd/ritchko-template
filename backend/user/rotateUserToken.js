...
module.exports = async function(param, req, res){
    ...
    // 토큰 rotate
    const token = await util.token.rotateTokenByRefreshToken(param.refreshToken);
    if(!token) return new response.RefreshTokenNotValid();

    return new response.RotateUserTokenOK(token)
}
